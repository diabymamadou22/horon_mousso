    ...
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-stone-200/90 shadow-[0_-4px_20px_rgba(0,0,0,0.06)] pb-safe">
      <div className="flex items-center justify-around h-16 max-w-lg mx-auto px-2">
          ...
          return (
            <button
              ...
              className={`flex-1 flex flex-col items-center justify-center py-1 relative min-h-[48px] rounded-xl transition-all cursor-pointer ${
                isActive 
                  ? 'text-[#0F2916] font-bold' 
                  : 'text-stone-500 hover:text-stone-900'
              }`}
            >
              <div className={`relative p-1 rounded-xl transition-all ${
                isActive ? 'bg-amber-100/70 text-[#0F2916] scale-110' : ''
              }`}>
                <Icon className={`w-5 h-5 transition-transform ${isActive ? 'stroke-[2.5] text-[#0F2916]' : 'stroke-2'}`} />
              </div>
              <span className={`text-[11px] leading-tight tracking-tight mt-0.5 ${
                isActive ? 'font-black text-[#0F2916]' : 'font-medium'
              }`}>
                {item.label}
              </span>
              {isActive && (
        ...
        <button
          ...
          className="flex-1 flex flex-col items-center justify-center py-1 relative min-h-[48px] text-stone-500 hover:text-stone-900 rounded-xl transition-all cursor-pointer"
          aria-label="Voir le panier"
        >
          <div className="relative p-1 rounded-xl">
            <ShoppingBag className="w-5 h-5 stroke-2" />
            {cartTotalCount > 0 && (
              <span className="absolute -top-1 -right-1.5 flex h-4.5 min-w-4.5 items-center justify-center rounded-full bg-[#B82B2B] px-1 text-[10px] font-black text-white shadow-xs animate-bounce">
                {cartTotalCount}
              </span>
            )}
          </div>
        ...
        <button
          ...
          className="flex-1 flex flex-col items-center justify-center py-1 relative min-h-[48px] text-emerald-800 hover:text-emerald-950 rounded-xl transition-all cursor-pointer"
          aria-label="Contacter sur WhatsApp"
        >
          <div className="relative p-1 rounded-xl bg-emerald-100/80 text-[#0F2916]">
            <MessageCircle className="w-5 h-5 stroke-[2.2]" />
          </div>
          <span className="text-[11px] font-bold leading-tight tracking-tight mt-0.5 text-[#0F2916]">
            WhatsApp
          </span>
        </button>
      </div>
